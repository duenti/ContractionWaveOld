...
public class ShowMultMarkerColorThickDialog {
	
	public static void redrawAnnotations(PackageData mainpack, XYPlot plot, XYPlot plot2, List<Integer> valid_maximum_list, List<Integer> valid_minimum_list,List<Integer> first_points, List<Integer> fifth_points, int zoomMinValue) {
		...
		if (valid_maximum_list.size() + valid_minimum_list.size() < 1500 && mainpack.getPlot_preferences().isDrawAnnotations() == true) {
	        for(int x1 = 0; x1 < dataset.getItemCount(0); x1++){
	        	...
	        	if (valid_maximum_list.contains(x1)) {
	        		plot.addAnnotation(new XYCircleAnnotation(x, y, 5.0, mainpack.getPlot_preferences().getMaximumDotColorRGB()));
	        	}
	        	if (valid_minimum_list.contains(x1)) {
	        		plot.addAnnotation(new XYCircleAnnotation(x, y, 5.0, mainpack.getPlot_preferences().getMinimumDotColorRGB()));
	        	}
	        	if (first_points.contains(x1) && !valid_minimum_list.contains(x1)) {
	        		plot.addAnnotation(new XYCircleAnnotation(x, y, 5.0, mainpack.getPlot_preferences().getFirstDotColorRGB()));
	        	}
	        	if (fifth_points.contains(x1) && !valid_minimum_list.contains(x1)  && !first_points.contains(x1)) {
	        		plot.addAnnotation(new XYCircleAnnotation(x, y, 5.0, mainpack.getPlot_preferences().getLastDotColorRGB()));
	        	}
	        }
		}
	...
	public static void showDialog(PackageData mainpack, XYPlot plot, XYPlot plot2, List<IntervalMarker> current_markers, List<Integer> valid_maximum_list, List<Integer> valid_minimum_list,List<Integer> first_points, List<Integer> fifth_points, int zoomMinValue) {
...
//	    GridPane.setValignment(buttonTypeOk, VPos.CENTER);
	    ...
	    buttonTypeOk.setOnAction(new EventHandler<ActionEvent>() {
	    	...
	    	public void handle(ActionEvent event) {
	        	...
	        	for (IntervalMarker m : current_markers) {
		            m.setPaint(newcolorm);
			        m.setAlpha(new_alpha);
	        	}
	        	ShowMultMarkerColorThickDialog.redrawAnnotations(mainpack, plot, plot2, valid_maximum_list, valid_minimum_list, first_points, fifth_points, zoomMinValue);
	        }
	    });
